	...
	UInt32* pDataSize, Boolean* pWriteable, void* pData)
{
	...
	case kAudioUnitProperty_ParameterList:               // 3,  listenable
    {
        ...
        // If there are no public parameters add dummy one
        if (mPublicParams > 0)
        {
        ...
        else
        {
            ...
            *pDataSize = sizeof(AudioUnitParameterInfo);
            if (pData)
            {
                ...
                pInfo->flags = kAudioUnitParameterFlag_CFNameRelease | kAudioUnitParameterFlag_HasCFNameString | kAudioUnitParameterFlag_IsReadable;
